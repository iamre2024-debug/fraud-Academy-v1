...
- The Decision and Luna reference replacement uses `src/SubmitDecisionPanel.jsx`, `src/LunaPostSubmissionPanel.jsx`, `src/DecisionReviewVisuals.jsx`, `src/displayDecisionLunaThemeV1.css`, and `src/displayDecisionLunaLayoutSafetyV1.css` for the focused final review, separate operational/finding choices, real evidence and notes routes, coded artwork, protected pre-submission lock, and personal/business post-submission coaching.
...
- `src/DirectCollapsibleText.jsx` is the reusable direct React compact-text wrapper; Active Tool purpose, expanded-record text, tray identifiers, Case Report packet text, notebook note entries, Navigation heading and Academy learning copy, Academy Progress package status, and Case Briefing transaction/payee and short-summary copy use it directly. The focused Decision/Luna reference cards use bounded native card copy.
...
- `src/LunaPostSubmissionPanel.jsx` owns the protected lock and post-submission reference debrief, follows the active built-in or generated personal/business case, and reads the saved learner package through the existing state boundaries.
