...
- Approved record cards use `src/displayInvestigationToolsThemeV1.css` on desktop/compact widths and the mission/mobile reference layers on phone pages; the retired table compatibility sheet stays absent.
...
- `src/InvestigationToolPanel.jsx` is the single active deep-tool and Case Report renderer. It keeps search, expansion, pinning, notes, neutral report packets, reviewed state, Timeline/Case Report routes, case switching, and Submit Decision routing on the existing action boundaries.
...
24. The Display Phase 4 guard protects the active desktop record cards and dedicated phone pages, preserves record actions, prevents required page overflow, and rejects restoration of the retired table compatibility layer.
