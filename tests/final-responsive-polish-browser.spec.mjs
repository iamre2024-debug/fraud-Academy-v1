...
test('all completed screens remain contained across the six approved responsive ranges', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== 'desktop-chromium', 'The desktop project drives the explicit six-range viewport matrix while existing screen suites retain Pixel 7 coverage.');
  ...
  // Generate + Open Case verifies the final layout against the IndexedDB-backed live catalog.
  ...
  expect(generatedCaseId).toBeTruthy();
