...
import { openWorkflowStage, runPaymentVerification, selectToolGroup } from './workspace-page-helpers.mjs';
...
async function openCoreTool(page, category, tool) {
  ...
  if (tool === 'Business 360') {
    const lookup = panel.getByRole('textbox', { name: 'Search Business 360' });
    const legalName = await panel.locator('.business-360-profile h3').first().innerText();
    await lookup.fill(legalName);
    await panel.getByRole('button', { name: 'Search business', exact: true }).click();
    await expect(panel.locator('.business-360-lookup-result')).toContainText('Exact fictional training record located.');
  }
