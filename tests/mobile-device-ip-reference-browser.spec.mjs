...
test('Device and IP Intelligence use dedicated mobile evidence pages without changing desktop', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('heading', { name: 'Device Intelligence', exact: true })).toBeVisible();
  ...
  const deviceId = (await devicePage.locator('.mobile-device-facts h3').innerText()).trim();
  await devicePage.getByRole('textbox', { name: 'Search Device Intelligence records' }).fill(deviceId);
  const moreDeviceEvidence = devicePage.locator('details.mobile-intel-more-panel');
  await moreDeviceEvidence.locator('summary').click();
  ...
  const deviceLayout = await page.evaluate(() => {
    ...
    const elements = [
      document.querySelector('.mission-workspace-bar'),
  ...
  await expect(page.getByRole('heading', { name: 'IP Intelligence', exact: true })).toBeVisible();
  ...
  await ipPage.getByRole('textbox', { name: 'Search IP Intelligence records' }).fill(ipAddress);
  ...
  const moreIpEvidence = ipPage.locator('details.mobile-intel-more-panel');
  await moreIpEvidence.locator('summary').click();
  ...
  await expect(ipPage.getByRole('button', { name: 'Mark IP Intelligence reviewed', exact: true })).toHaveCount(0);

  const ipLayout = await page.evaluate(() => {
    const viewport = window.innerWidth;
    const elements = [
      document.querySelector('.mission-workspace-bar'),
      document.querySelector('.mission-ip-intelligence-page .mobile-ip-summary'),
