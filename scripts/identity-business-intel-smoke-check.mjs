...
requireText('Business360Workspace.jsx', business, 'readStorage');
requireText('Business360Workspace.jsx', business, 'writeStorage');
requireText('Business360Workspace.jsx', business, 'fraud-academy-business-intel-workspace-v1');
requireText('Business360Workspace.jsx', business, "setQuery?.('')");
requireText('Business360Workspace.jsx', business, 'String(query).trim() || intelWorkspace.relationshipId');
requireText('Business360Workspace.jsx', business, 'relationshipId: intelWorkspace.relationshipId');
requireText('Business360Workspace.jsx', business, "query={intelWorkspace.relationshipId || submittedSearch?.secondary || query}");
requireText('Business360Workspace.jsx', business, 'data-business-intelligence-stage="search"');
requireText('Business360Workspace.jsx', business, 'data-business-intelligence-stage="business-360"');
requireText('InvestigationToolPanel.jsx', panel, 'setQuery={setQuery}');
...
requireText('identity-business-intel-browser.spec.mjs', browser, 'routed Business ID stays cleared after leaving and reopening Business Intelligence');
requireText('identity-business-intel-browser.spec.mjs', browser, 'new phone search replaces a routed Business ID after leaving and reopening Business Intelligence');
...
const businessIdSearch = {
  mode: 'businessId',
  businessName: profile.legalName,
  secondary: profile.businessId,
};
if (!matchesBusinessIntelSearch(dossier, businessIdSearch)) {
  fail('the canonical business ID does not resolve the Business Intelligence record.');
}
const businessIdPrefill = prefillBusinessIntelSearch(dossier, profile.businessId);
if (!businessIdPrefill || businessIdPrefill.mode !== 'businessId' || !matchesBusinessIntelSearch(dossier, businessIdPrefill)) {
  fail('a routed canonical business ID does not restore the Business Intelligence search.');
}
