...
assert.equal(
  ...
  'Financial Investigation:Account ID:ACCT-42',
  'Quick Pad item identity must migrate with its canonical source alias.',
);
...
const lateContextDraft = migrateDecisionDraft(
  JSON.parse(localStorage.getItem(keyNames.decisions))[lateContextCaseId],
  {
...
assert.equal(
  lateContextDraft.operationalDecision,
  'Deny',
  'A late-loaded record must be normalized at its load boundary without rerunning the global migration.',
);
...
assert.equal(
  ...
  'Business 360:Business ID:BIZ-42',
);
assert.deepEqual(
