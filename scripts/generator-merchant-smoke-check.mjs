...
for (const { claimType, scenario } of allScenarios) {
  ...
  if (generated.availableTools.includes('Business 360')) {
    const businessWorkspace = getBusinessResearch(generated);
    if (!businessResearchSections.every((section) => businessWorkspace.recordsBySection[section.id]?.length)) {
      failures.push(`${scenario.id} is missing a complete reusable Business 360 profile.`);
    }
  }
  ...
  if (
    ...
    && (
      [...builtIn.availableTools, ...builtIn.requiredTools]
        .some((tool) => ['Employee Profile', 'Payroll History'].includes(tool))
        && !hasPayrollRelationship(builtIn)
      || (
...
if (
  isWorkflowEnabled('business', PRODUCT_TYPES.PAYROLL_PRODUCT, WORKFLOW_TYPES.CREDIT_APPLICATION_REVIEW)
  || isWorkflowEnabled('business', PRODUCT_TYPES.PAYROLL_PRODUCT, WORKFLOW_TYPES.CREDIT_RISK_REVIEW)
) {
  failures.push('Payroll Product still enables a retired credit workflow.');
}
...
if (
  !builtInCreditReview
  || builtInCreditReview.customerType !== 'personal'
  || builtInCreditReview.productType !== PRODUCT_TYPES.PERSONAL_LOAN
  || builtInCreditReview.workflowType !== WORKFLOW_TYPES.CREDIT_RISK_REVIEW
  || !['Business 360', 'Employee Profile', 'Payroll History']
    .every((tool) => builtInCreditReview.availableTools.includes(tool))
  || builtInCreditReview.availableTools.includes('KYB Review')
  || !hasPayrollRelationship(builtInCreditReview)
  || !hasOwnershipLinkedBusinessRelationship(builtInCreditReview)
) {
  failures.push('FA-CR-24003 must preserve its personal credit-risk taxonomy and explicit linked-employer payroll tools.');
}
...
const businessProfile = getBusinessResearch(businessApplication);
const serializedBusinessProfile = JSON.stringify(businessProfile);
if (serializedBusinessProfile.includes(businessApplication.id) || serializedBusinessProfile.includes(businessApplication.caseBriefing?.summary)) {
  failures.push('Business 360 contains current application identifiers or summary.');
}
if (!JSON.stringify(getFinancialInvestigation(businessApplication)).includes(businessRevenue)) {
  failures.push('Business Financial Investigation does not retain the stated revenue comparison.');
}
