...
assert(
  JSON.stringify(legacyPackage.requiredTools) === JSON.stringify(['Case Summary', 'Document Viewer', 'Business 360'])
...
assert(
  frozenDisplaySnapshot.pinnedEvidence.length === 0
    && frozenDisplaySnapshot.noteSnapshot.length === 0,
  'Explicitly empty saved pin and note snapshots should remain empty after live case mutations.',
);
assert(
