...
2. Continue splitting `VisualWorkspace.jsx` into smaller React modules so compact text can move away from selector-discovery compatibility. The workspace model, active tool panel, bottom investigation grid, case summary card, category rail, and Submit Decision panel are already split.
