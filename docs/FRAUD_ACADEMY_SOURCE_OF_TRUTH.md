...
- `src/DesktopMissionControlApp.jsx` and `src/desktopMissionControlV2.css` own the direct desktop command center: four primary routes, contextual Progress/Profile, full-width wrapping cards, and exact Timeline, Pinned Evidence, Case Notes, and Tool Library shortcuts. The desktop migration changes no persistence schema, environment variable, Evidence First rule, or mobile renderer.
- `docs/FRAUD_ACADEMY_DESKTOP_MISSION_CONTROL_V2.md` records the desktop runtime composition, routing map, no-schema migration behavior, and desktop/mobile verification boundary.
...
- `src/data/resumeSession.js` owns validation for the last active navigation tab, case, workspace page/tool, and Luna debrief location. Resume state joins the encrypted cloud snapshot as one last-writer-wins value, while `fraud-academy-layout-mode-v1` remains device-local.
...
- `tests/cloud-persistence-browser.spec.mjs` verifies desktop and Pixel 7 exact case/page/tool close/reopen recovery, clean-device recovery-code restore, generated-scenario cloud recovery, offline Quick Pad and navigation saves, and safe retry after a forced revision conflict.
