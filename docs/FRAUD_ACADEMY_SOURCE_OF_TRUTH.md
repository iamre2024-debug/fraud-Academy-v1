...
- `src/data/accessHistoryReports.js` owns the Login Timeline, Session History, and IP Intelligence system reports. `src/data/documentRecords.js` exposes generated access-history reports under Document Viewer → System Reports without moving determination flags or conclusions into an investigation tool.
...
- `src/BottomInvestigationGrid.jsx` owns the Investigation Tray and Investigation Notebook cards, including pinned objects, Open Document Viewer routing, note compose/save UI, Case Report packet feed, and notebook list. Preserve case-scoped props and ornate tray/notebook classes.
...
- The old `src/visualInvestigationRepair.js` route patch is retired. Do not restore DOM repair logic for Device IDs, Case Briefing metadata, Submit Decision routing, Tool Map, or Open Document Viewer.
