...
- `src/CaseSummaryCard.jsx` owns the ornate Case Summary card, including neutral intake metadata, Pin Case, quick tool routes, and Submit Decision jump. Keep the summary based on allegation/system-alert context only and do not reveal outcome.
...
- `src/DirectCollapsibleText.jsx` is the reusable direct React compact-text wrapper. New module work should prefer this wrapper over selector discovery; `src/VisualTextCollapse.jsx` remains a limited compatibility layer for remaining legacy targets and must ignore direct compact-text targets. Do not place direct More / Less controls inside another `<button>`; refactor the parent control first.
...
- Browser localStorage is not truly unlimited. Any migration to IndexedDB or a backend must preserve existing locally generated cases, case IDs, and case-scoped workspace state, and requires explicit architecture approval before implementation.
...
- `npm run verify` must preserve the Evidence First wording check, functional smoke guard, visual three-case smoke guard, generated-case smoke guard, Luna single-module smoke guard, review-package behavior smoke check, and production build so CI catches broken visual-shell anchors, case coverage, generated queue regressions, direct route callbacks, Luna duplication, and package-lock behavior before they ship.
