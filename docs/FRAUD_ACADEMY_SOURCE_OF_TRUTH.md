...
After Display Phase 3 passes its full verify and browser gates, begin only Phase 4 from `docs/FRAUD_ACADEMY_DISPLAY_HANDOFF.md`: replace dense mobile record overflow with a touch-friendly no-horizontal-scroll presentation without changing Evidence First, Luna gating, generated-case persistence, or the System Access boundary.
