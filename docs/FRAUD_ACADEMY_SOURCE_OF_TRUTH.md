...
Display Phase 4's responsive contract now follows the approved record-card renderer and dedicated mobile pages. The active layers use the existing record data and actions, add no required horizontal page scroller, and do not touch case persistence, generated-case storage, Evidence First, Luna gating, or System Access architecture.
...
- `src/displayInvestigationToolsThemeV1.css` owns the active desktop and compact-width record-card presentation, `src/mobileMissionDeckV3.css` owns the shared focused-tool mobile shell, and the dedicated mobile reference styles own their tool-specific pages. The retired table compatibility layer must not return; these active owners keep controls thumb-friendly and must not add required horizontal page scrolling, fixed overlays, persistence access, or investigation behavior.
...
- `src/InvestigationToolPanel.jsx` is the single active deep-tool and Case Report renderer. Keep Evidence First copy and the existing record/action behavior intact; do not restore the retired duplicate panel.
