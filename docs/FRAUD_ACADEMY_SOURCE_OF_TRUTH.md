...
- `src/DirectCollapsibleText.jsx` is the reusable direct React compact-text wrapper. Active Tool content, notebook/tray text, Submit Decision checklist messages, Luna coaching, Navigation heading, Academy learning copy, and Progress package status use it directly. New module work should prefer this wrapper over selector discovery; `src/VisualTextCollapse.jsx` remains a limited compatibility layer for remaining legacy summary targets and must ignore direct compact-text targets. Do not place direct More / Less controls inside another `<button>`; refactor the parent control first.
...
- Compact More / Less controls should be React-owned through `src/DirectCollapsibleText.jsx`; keep long copy collapsed by default. `src/VisualTextCollapse.jsx` is only a limited compatibility layer for remaining legacy summary targets, and the retired `src/visualTextCollapse.js` patch must not return.
