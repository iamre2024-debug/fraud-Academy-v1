...
Includes Transaction History, Financial Investigation, Payment Verification, destination records, balance behavior, merchant/payment context, and account activity patterns.

### Business

Investigator question: Is the business, employee, payroll, or merchant relationship real?

Includes Business 360, KYB Review, Employee Profile, Payroll History, merchant records, KYB-style review, and business relationship verification.

### Evidence

Investigator question: What evidence do I have, what is missing, and what supports the final decision?

Includes Document Viewer, customer documents, platform records, uploaded items, system records, and generated case packets.
...
- `src/investigationToolGroups.js` owns the six approved contextual groups: Identity & Customer; Login, Session, Device & IP; Transactions & Financial; Business & Payment Verification; Documents & Requests; and Links & Related Cases. Timeline now uses its approved dedicated renderer, while Case Report remains in Workflow Review until its later isolated redesign.
...
- `src/data/loginRecords.js`, `src/data/sessionRecords.js`, and `src/data/ipRecords.js` own the linked access-history views. Login History is authentication-only; Session History begins with successful authentication and owns post-login activity; IP Intelligence requires an exact lookup before network detail is displayed. `src/data/accessHistoryUtils.js` owns their shared timestamp and deterministic generated-record helpers.
- `src/data/accessHistoryReports.js` owns the Login Timeline, Session History, and IP Intelligence system reports. `src/data/documentRecords.js` exposes generated access-history reports under the matching Account ID in Document Viewer without moving determination flags or conclusions into an investigation tool.
- `src/TimelinePanel.jsx` owns the approved Timeline workspace: neutral event metrics, search, source filtering, sequence cards, expanded source review, pinning, notes, neutral report packets, reviewed state, and routes to Document Viewer, Case Report, and locked Submit Decision. `src/displayTimelineThemeV1.css` owns only its responsive light-lavender and white presentation. Neither file may access generated-case persistence, expand System Access, or reveal an outcome before submission.
- `src/SubmitDecisionPanel.jsx` owns the reference Final Review workspace: active-case identity, selected operational action and separate final finding, workflow-valid editing, confidence, evidence-based finding basis, pinned-evidence preview, latest note, package saving, and Luna unlock. It must remain Evidence First and must not expose Luna scoring, answer guidance, or a case conclusion before submission.
- `src/LunaPostSubmissionPanel.jsx` owns the reference locked and post-submission Luna states. It resolves the active built-in or generated personal/business case, refreshes immediately from the case-scoped package-saved event, shows no outcome or coaching while locked, and displays strengths, missed evidence, workflow risk guidance, and motivation only after a learner package exists.
- `src/DecisionReviewVisuals.jsx` owns the coded Luna mascot, lighthouse medallion, and review glyphs. `src/displayDecisionLunaThemeV1.css` owns the scoped midnight-blue/cyan/pink reference presentation. `src/displayDecisionLunaLayoutSafetyV1.css` owns only focused viewport containment. These files may not access persistence, expand System Access, or add fixed review overlays.
...
- `src/visualWorkspaceModel.js` owns workspace constants, storage helpers, row builders, and Case Report packet construction so `src/VisualWorkspace.jsx` can keep shrinking into focused React modules without changing the screenshot-driven shell. `src/data/persistenceKeys.js` is the centralized storage-key registry used by local and cloud persistence.
- `src/useVisualWorkspaceCaseState.js` owns the case-scoped browser-persistence slices for tray items, notes and note drafts, reviewed tools, decision drafts, learner packages, action history, document requests, and Quick Pad state. Keep generated-case persistence separate behind `src/data/generatedCaseRepository.js`.
...
- `src/BottomInvestigationGrid.jsx` owns the Investigation Tray and Investigation Notebook cards, including pinned objects, note compose/save UI, and notebook list. Document Viewer has no shortcut from this or any other workspace page; investigators open it from the Evidence tool group and search by Account ID.
...
- `src/DirectCollapsibleText.jsx` is the reusable direct React compact-text wrapper. Active Tool content, notebook/tray text, Case Report packet text, Navigation heading, Academy learning copy, Progress package status, and Case Briefing long copy use it directly. The focused Decision/Luna reference cards use intentionally bounded native card copy instead of selector discovery. Do not place direct More / Less controls inside another `<button>`; refactor the parent control first.
...
- Visual-shell Investigation Tray, case notes and drafts, reviewed tools, decision drafts, review packages, action history, document requests, Quick Pad state, and completed Luna debriefs persist in browser storage by case. The cloud adapter mirrors those established values without replacing their immediate local recovery.
...
- Luna outcomes, strengths, missed-evidence coaching, risk guidance, motivation, and any underlying quality calculation stay hidden until a review package is saved.
...
- The old `src/visualInvestigationRepair.js` route patch is retired. Do not restore DOM repair logic for Device IDs, Case Briefing metadata, Submit Decision routing, Tool Map, or Document Viewer shortcuts.
- `src/data/coreToolRecords.js` is a focused display overlay for deeper Payment Verification, KYB Review, Document Viewer, Link Analysis, Timeline, and Case Report records. It must not replace `visualWorkspaceModel.js`, browser case state, or the generated-case repository boundary.
...
- `tests/decision-luna-browser.spec.mjs` verifies the reference Decision & Luna flow on desktop and Pixel 7 for personal and business cases, including Evidence First locking, separate action/finding choices, confidence, rationale, saved-package submission, immediate and persisted Luna unlock, workflow coaching, share/back actions, and viewport containment.
- `tests/cloud-persistence-browser.spec.mjs` verifies desktop and Pixel 7 close/reopen recovery, clean-device recovery-code restore, generated-scenario cloud recovery, offline Quick Pad saves, and safe retry after a forced revision conflict.
