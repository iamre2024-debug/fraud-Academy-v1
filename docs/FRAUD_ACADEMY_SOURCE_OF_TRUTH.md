...
- `docs/FRAUD_ACADEMY_TIMELINE_THEME_V1.md` is the approved Timeline handoff. It records case-scoped event ordering, neutral source verification, preserved evidence actions and routes, and responsive verification.
- `docs/FRAUD_ACADEMY_DECISION_LUNA_THEME_V1.md` is the approved Decision and Luna handoff. It records the Evidence First determination workspace, case-scoped learner-package submission, locked pre-submission Luna state, post-submission debrief, preserved routes and persistence boundaries, responsive verification, and the Academy-only next step.
- `src/VisualApp.jsx` coordinates the active case, live case catalog, and visual navigation tab through React state so case switching, generated-case opening, Tool Map, Open Evidence Center, and Submit Decision routing do not depend on DOM repair scripts. It mounts the Cases replacement against the same live built-in/generated catalog and existing `onOpenCase` route.
...
- `src/SubmitDecisionPanel.jsx` owns the approved Evidence First determination workspace: readiness metrics, final evidence checklist, lane-organized learner choices, confidence, rationale progress, saved learner-package confirmation, and the existing submission action boundary. `src/LunaPostSubmissionPanel.jsx` owns the approved locked pre-submission state and saved-package debrief routes. `src/displayDecisionLunaThemeV1.css` and `src/displayDecisionLunaLayoutSafetyV1.css` own only their responsive light-lavender and white presentation and viewport containment. These files must not access generated-case persistence, change storage keys, reveal answer guidance before submission, or absorb Academy or Profile redesign work.
...
- `tests/decision-luna-browser.spec.mjs` verifies the approved Decision and Luna surfaces on desktop and Pixel 7, including Evidence First locked copy, learner choice, confidence, rationale, package saving, post-submission debrief, active-case persistence after refresh, route controls, stacked mobile layouts, and viewport safety.
...
- `.github/workflows/build.yml` must also run Playwright against desktop and mobile Chromium for the approved Dashboard, Cases queue, Workspace shell, Case Briefing, Customer 360, Investigation tools, Timeline, and Decision & Luna, all three built-in cases, generated-case immediate open and reload persistence, the completed core modules, Connections → System Access Lane, Luna’s pre-submission lock, the active-case workflow surface, and responsive record presentation.

## Next safe item

The approved Dashboard, Cases, Workspace shell, Case Briefing, Customer 360, Investigation tools, Timeline, and Decision & Luna redesigns are complete and protected by focused static and desktop/mobile browser guards. The next isolated safe item is **Academy only**. Preserve the four-item navigation, contextual Academy Progress access, saved-package refresh behavior, active-case return routes, Evidence First, Luna's pre-submission lock, IndexedDB-first generated-case persistence, every storage key, and the single Connections → System Access Lane. Do not begin Profile or final responsive/mobile redesign in the same pull request.
