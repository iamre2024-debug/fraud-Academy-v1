    ...
    <main className="visual-os-shell">
      <section className="visual-os-frame">
        ...
        <section className="ornate-card activity-panel">
          ...
          <div className="tool-purpose-card"><strong>{tool}</strong><p>{tool === 'Device Intelligence' ? 'Device IDs help separate repeated known devices from new devices. Repeated device names keep the same fictional Device ID for this customer.' : 'Review available case records while keeping the final decision locked.'}</p><div className="tool-flow-chips">{workflows.map((item) => <span key={item}>{item}</span>)}</div><button type="button" className="decision-route-mini" onClick={jumpDecision}>Need to decide? Open Submit Decision</button></div>
