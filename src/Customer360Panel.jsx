...
const unavailable = 'No record supplied';
...
function firstDetail(items = [], index = 0, fallback = unavailable) {
  return items[index]?.value ?? fallback;
}
...
function buildDossierSections(activeCase) {
  ...
  const associates = (profile.associates ?? []).map((item) => item.value).join(' · ') || unavailable;
  const businesses = (profile.businesses ?? []).map((item) => item.value).join(' · ') || unavailable;
  const priorCases = (profile.priorCases ?? []).map((item) => `${item.value}: ${item.detail}`).join(' · ') || 'No separate prior case record supplied';
  ...
  return [
    {
      ...
      subtitle: 'Name, Training ID, DOB, and relationship details',
      fields: [
        ['Name', activeCase.person ?? unavailable],
        ['Training ID', activeCase.trainingId ?? unavailable],
        ['DOB / age', profile.dob ? `${profile.dob} · ${profile.age}` : unavailable],
    ...
    },
    {
      ...
      subtitle: 'Current and historical contact points',
      fields: [
        ['Mobile phone', contact.phone ?? unavailable],
        ['Alternate phone', firstDetail(profile.phones, 1)],
        ['Email', contact.email ?? unavailable],
        ['Alternate email', firstDetail(profile.emails, 1)],
        ['Mailing address', contact.address ?? unavailable],
        ['Prior address', firstDetail(profile.addresses, 1)],
    ...
    },
    {
      ...
      subtitle: 'Products, payment profile, exposure, and standing',
      fields: [
    ...
    },
    {
      ...
      subtitle: 'Normal behavior and linked relationship context',
      fields: [
        ['Normal transaction context', activeCase.transactionInfo ?? unavailable],
        ['Normal login location', relationshipValue(activeCase, 'Normal login area')],
        ['Observed devices', knownDevices.join(' · ') || unavailable],
        ['Associates / household links', associates],
        ['Business or employer associations', businesses],
        ['Linked profiles', (profile.linkedProfiles ?? []).map((item) => item.value).join(' · ') || unavailable],
      ],
    },
    {
      id: 'security',
      ...
      subtitle: 'Neutral access facts from the customer relationship',
      fields: [
        ['MFA / login methods', methods.join(' · ') || unavailable],
        ['Password last changed', profile.passwordLastChanged ?? unavailable],
    ...
    },
    {
      ...
      subtitle: 'Intake, callback, and document-contact context',
      fields: [
        ...
        ['Recent profile events', `${activeCase.customer?.profileChanges?.length ?? 0}`],
      ],
    },
    {
      id: 'prior-claims',
      icon: 'HIST',
      title: 'Prior Claims / Service History',
      subtitle: 'Historical customer context without an outcome inference',
      fields: [
        ['Prior case records', priorCases],
        ['Prior merchant / account history', activeCase.facts?.find((item) => /merchant|account|history/i.test(item)) ?? 'Review related tool records'],
        ...
        {mobilePage === 'related-records' && (
          <>
            ...
            <label className="customer-360-page-search">
              ...
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search customer records" aria-label="Search related customer records" />
            </label>
            <div className="customer-360-page-record-list">
              {filteredRows.map((row) => {
                ...
                return (
                  <article key={row.id} className={open ? 'open' : ''}>
                    <button type="button" className="customer-360-record-open" onClick={() => { setSelectedRecordId(open ? '' : row.id); setExpandedId(open ? '' : row.id); }}>
                      <span>{row.id}</span>
              ...
              {!filteredRows.length && <p className="customer-360-page-empty">No related records match the current search.</p>}
            </div>
          </>
        )}
      </section>
  ...
  return (
    <section className="ornate-card activity-panel customer-360-theme-v1 customer-360-overview" data-customer-360-screen="approved-theme-v1">
      <header className="customer-360-header">
        <div>
          <p className="customer-360-eyebrow">Customer relationship · Evidence First</p>
          <h2>Customer 360</h2>
          <p>Open one customer record area at a time. Detailed background searches remain inside Identity Intelligence.</p>
        </div>
      ...
      <section className="customer-360-identity-band" aria-label="Customer summary">
        ...
        <div className="customer-360-identity-copy">
          ...
          <p>{activeCase.trainingId} · DOB {activeCase.identityProfile?.dob ?? unavailable} · Customer since {activeCase.customer?.relationshipSince ?? 'not listed'}</p>
        </div>
