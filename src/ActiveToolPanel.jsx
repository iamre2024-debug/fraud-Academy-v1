...
export default function ActiveToolPanel({
  ...
  const selectedId = expandedId || activeRow?.id;
  ...
  return (
    <section className="ornate-card activity-panel">
      ...
      <div className="activity-table"><div className="activity-row table-head">{displayData.columns.map((item) => <span key={item}>{item}</span>)}</div>{displayRows.map((row) => <div key={row.id} className={`activity-row ${displayActiveRow?.id === row.id ? 'expanded' : ''}`}>{row.values.map((value, index) => <span key={`${row.id}-${index}`}>{lineBreaks(value)}</span>)}<div className="row-action-group"><button type="button" className="row-expand-button" onClick={() => setExpandedId(row.id)}>Expand</button><button type="button" className="row-pin-button" onClick={() => pin(row.pin)}>📌</button></div></div>)}</div>
