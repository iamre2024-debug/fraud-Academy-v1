...
export default function ActiveToolPanel({
  ...
  return (
    <section className="ornate-card activity-panel">
      ...
      <div className="tool-purpose-card"><strong>{tool}</strong><DirectCollapsibleText lines={2}>{purposeFor(tool)}</DirectCollapsibleText><div className="tool-flow-chips">{workflows.map((item) => <span key={item}>{item}</span>)}</div><button type="button" className="decision-route-mini" onClick={jumpDecision}>Need to decide? Open Submit Decision</button></div>
      ...
      {activeRow && <section className="record-detail-panel"><div className="record-detail-heading"><div><span>Expanded Record</span><h3>{activeRow.id}</h3></div><button type="button" onClick={() => saveNote(`Expanded ${tool} record ${activeRow.id}: ${activeRow.detail}`, 'Expanded record')}>Save expanded note</button></div><div className="record-review-lanes"><article><h4>History</h4><DirectCollapsibleText>☾ {activeRow.id} is open inside {tool} for {activeCase.id}.</DirectCollapsibleText><DirectCollapsibleText>☾ Record history can be compared with pinned evidence, timeline entries, and the case report draft.</DirectCollapsibleText></article><article><h4>Link Analysis</h4><DirectCollapsibleText>⌘ {activeRow.label}: {activeRow.pin}</DirectCollapsibleText><DirectCollapsibleText>⌘ Case object: {activeCase.id} · {activeCase.person}</DirectCollapsibleText></article><article><h4>Generated Report</h4><DirectCollapsibleText>✦ Source tool: {tool}.</DirectCollapsibleText><DirectCollapsibleText>✦ Record summary: {activeRow.detail}.</DirectCollapsibleText><div className="record-report-actions"><button type="button" onClick={() => saveCaseReportPacket(activeRow)}>Save neutral report packet</button><button type="button" onClick={() => markReviewed(tool)}>{currentCompleted.includes(tool) ? 'Reviewed' : 'Mark reviewed'}</button></div></article></div></section>}
