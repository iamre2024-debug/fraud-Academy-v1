  ...
  return (
    <section className="ornate-card activity-panel">
      <div className="activity-heading">
        ...
        <span className="panel-cat" aria-hidden="true">🐈‍⬛</span>
      </div>

      <div className="tool-purpose-card">
        <strong>{tool}</strong>
        <DirectCollapsibleText lines={2}>{purposeFor(tool)}</DirectCollapsibleText>
        <div className="tool-flow-chips">{workflows.map((item) => <span key={item}>{item}</span>)}</div>
      ...
      <div className="workspace-search-row">
        <input
          ...
          placeholder="Search records, history, device, IP, merchant, document..."
      ...
      <div className="activity-table" role="table" aria-label={`${tool} records`}>
        ...
        {displayRows.map((row) => (
          <div
            key={row.id}
            className={`activity-row ${displayActiveRow?.id === row.id ? 'expanded' : ''}`}
            role="row"
          ...
          >
            {row.values.map((value, index) => (
              <span
                key={`${row.id}-${index}`}
                role="cell"
                data-field={displayData.columns[index] ?? `Field ${index + 1}`}
              >
                {lineBreaks(value)}
              </span>
            ))}
            <div className="row-action-group" role="cell" data-field="Actions">
              <button type="button" className="row-expand-button" onClick={() => expandRecord(row.id)}>Expand</button>
        ...
        {!displayRows.length && (
          <div className="activity-empty-state" role="status">
            No records match the current search. Clear or revise the search to continue reviewing this tool.
          </div>
        )}
      </div>
      ...
      {displayActiveRow && (
        <section className="record-detail-panel">
          <div className="record-detail-heading">
            <div><span>Expanded Record</span><h3>{displayActiveRow.id}</h3></div>
          ...
          <div className="record-review-lanes">
            ...
            <article>
              ...
              <div className="record-report-actions">
                ...
                <button type="button" onClick={() => markReviewed(tool)}>{currentCompleted.includes(tool) ? 'Reviewed' : 'Mark reviewed'}</button>
              </div>
            </article>
          </div>
        </section>
      ...
      <button type="button" className="view-full-button" onClick={() => markReviewed(tool)}>
        {currentCompleted.includes(tool) ? '✓ Reviewed · Generate Another Neutral Tool Report' : '✦ Generate Neutral Tool Report ›'}
      </button>
    </section>
  );
}
