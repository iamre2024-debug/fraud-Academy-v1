...
function IndicatorRow({ answer, item, updateDecisionIndicator }) {
...
export function MobileDeterminationPage({
  ...
  return (
    ...
    >
      ...
      <form className="mobile-determination-form decision-v1-workspace" onSubmit={submitAndOpenDebrief}>
        <section className="mobile-review-card mobile-evidence-summary">
          ...
          <dl>
            ...
            <div><dt><ReviewGlyph type="pin" size={18} />Learner inputs</dt><dd>{tray.length} pins · {notes.length} notes · {packageStatus.indicatorSummary.selectedCount} indicators</dd></div>
          </dl>
            ...
            <fieldset key={group.label}>
              ...
              <div className="mobile-choice-grid">
                {group.options.map((item, index) => (
                  <label key={item} data-selected={decisionDraft.operationalDecision === item ? 'true' : 'false'} data-tone={choiceTone(item, index)}>
                    <input
                      type="radio"
          ...
          <fieldset>
            ...
            <div className="mobile-choice-grid">
              {findings.map((item, index) => (
                <label key={item} data-selected={decisionDraft.finalFinding === item ? 'true' : 'false'} data-tone={choiceTone(item, index)}>
                  <input
                    type="radio"
        ...
        <p className="mobile-decision-submit-help" role="status">
          {packageStatus.ready
            ? 'Ready to save this learner package and unlock Luna.'
            : `Before submitting: ${packageStatus.blockers.join('; ')}.`}
        </p>
      </form>

      {latestPackage && (
        <section className="mobile-review-card mobile-decision-confirmation" role="status" aria-label="Decision submission confirmation">
          <span><ReviewGlyph type="check" size={22} /></span>
          <div>
            <small>Saved learner package</small>
            <h3>{latestPackage.operationalDecision || latestPackage.choice}</h3>
            <p>{latestPackage.finalFinding || 'Finding not recorded'} · {latestPackage.confidence} confidence</p>
          </div>
          <button type="button" onClick={() => openDebrief?.(latestPackage)}>Open Luna</button>
        </section>
      )}
    </section>
  );
