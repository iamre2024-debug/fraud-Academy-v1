...
function buildClaimFields(item, context = {}) {
  ...
  const availableTools = filterToolsForCaseDomain(
    hasOwnershipLinkedBusinessRelationship(domain)
      ? canonicalToolNames(dedupeStrings([...availableToolCandidates, 'Business 360']))
      : canonicalToolNames(availableToolCandidates),
    domain,
  );
  ...
  return {
    ...
    requiredTools: canonicalToolNames(requiredTools),
