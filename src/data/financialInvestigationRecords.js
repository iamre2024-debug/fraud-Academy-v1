...
function normalizePayrollRun(activeCase, run, index, asOfValue) {
  ...
  const netPayroll = payrollMetric(run, ['netPayroll', 'netPay']);
  const fundingAmount = payrollMetric(run, [
    'fundingAmount',
    'fundingDebit',
    'totalFunding',
    'totalFundingAmount',
  ...
  return createRecord({
    id,
    title: `${safeEvidenceText(
      run.employer
      ?? activeCase.toolResults?.companyPayrollProfile?.legalName
      ?? activeCase.profile?.business,
      'Training business',
    )} · ${safeEvidenceText(run.payPeriod?.label ?? run.period, range.label)}`,
    category: 'Payroll run',
    value: companyDebit === null ? unavailable : formatMoney(companyDebit),
    observed: safeEvidenceText(run.payDate ?? run.period ?? run.effectiveDate, range.label),
    status: runStatus,
    ...
    fields: [
      ...
      ['Funding source', safeEvidenceText(
        run.fundingSource
...
function buildPayroll(activeCase, asOfValue) {
  const expected = activeCase.customerType === CUSTOMER_TYPES.BUSINESS && (
    ...
    || Boolean(activeCase.toolResults?.payrollHistory?.length)
  );
  if (!expected) {
    return {
      records: [],
  ...
  const records = (Array.isArray(payrollHistory) ? payrollHistory : payrollHistory.payrollRuns ?? [])
