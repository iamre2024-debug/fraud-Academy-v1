...
export function mergeCloudSnapshots(left = {}, right = {}) {
  ...
  return {
    ...
    generatedCases: {
      mode: 'array',
      // Merge the migrated records so a merged snapshot carrying the current
      // schema flag cannot reintroduce embedded hidden truth from a raw peer.
      // The case-domain merge still retains the left/local worked evidence.
      items: mergeGeneratedItems(
        migratedLeft.generatedCases?.items,
        migratedRight.generatedCases?.items,
      ),
    },
    generatedCaseTruth: {
      mode: 'array',
      items: mergeGeneratedTruthItems(
        migratedLeft.generatedCaseTruth?.items,
        migratedRight.generatedCaseTruth?.items,
      ),
    },
  };
}
