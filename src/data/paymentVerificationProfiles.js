...
const builtInProfiles = {
  'FA-ATO-24018': {
    destinationId: 'DST-4410-7784',
    bankCode: 'BC-110-ATO',
    ownershipName: 'Maya Sterling',
    ownershipComparison: 'The fictional destination-owner field matches Maya Sterling. The linked debit card and merchant processor remain available in the detailed payment records.',
    accountType: 'Traditional bank checking destination linked to debit card ending 4410',
    verificationStatus: 'Open · verification history available',
    verificationHistory: [
      { value: 'Jul 8, 2026 · DST-4410-7784 verified', detail: 'Destination ID and Bank Code were returned with the customer-owner field' },
      { value: 'Jul 8, 2026 · AUTH-74218-CNP', detail: 'Authorization trail linked to TXN-1001 and processor token MPT-7784' },
      { value: 'Jan 18, 2026 · card replacement', detail: 'Prior servicing record connected to debit card ending 4410' },
    ],
    priorPayrollUse: [{ value: 'No payroll use tied to this destination', detail: 'Payroll History contains no customer payroll destination for this consumer card case' }],
    sharedUse: [{ value: 'No separate joint-owner record supplied', detail: 'The current payment packet lists Maya Sterling as the fictional destination owner' }],
  },
  'FA-CB-24007': {
    destinationId: 'DST-8841-2207',
    bankCode: 'BC-220-CB',
    ownershipName: 'Jordan Ellis',
    ownershipComparison: 'The fictional destination-owner field matches Jordan Ellis. The linked credit card and recurring merchant token remain available in the detailed payment records.',
    accountType: 'Traditional bank payment destination linked to credit card ending 8841',
    verificationStatus: 'Open · recurring billing packet available',
    verificationHistory: [
      { value: 'Jul 8, 2026 · DST-8841-2207 verified', detail: 'Destination ID and Bank Code were returned with the customer-owner field' },
    ...
    priorPayrollUse: [{ value: 'No payroll use tied to this destination', detail: 'Payroll History contains no customer payroll destination for this billing dispute' }],
    sharedUse: [{ value: 'No authorized-user record supplied', detail: 'Do not infer shared use without an attached account-user record' }],
  },
  'FA-CR-24003': {
    ...
    verificationStatus: 'Open · pending investigator review',
    verificationHistory: [
...
function generatedProfile(activeCase) {
  ...
  return {
    ...
    verificationStatus: 'Open · generated verification packet available',
    verificationHistory: [
...
export function buildPaymentVerificationProfile(activeCase) {
  ...
  const paymentRows = financial?.paymentVerification ?? [
    { id: `${activeCase.id}-PAY-1`, type: 'Destination ID', object: base.destinationId, status: base.verificationStatus, lastSeen: activeCase.opened, context: base.ownershipComparison },
    { id: `${activeCase.id}-PAY-2`, type: 'Bank Code', object: base.bankCode, status: base.verificationStatus, lastSeen: activeCase.opened, context: base.accountType },
  ];
  const transactions = financial?.transactions ?? [
    { id: `${activeCase.id}-TXN-1`, posted: activeCase.opened, time: 'Generated case time', merchant: activeCase.transactionInfo, amount: activeCase.amount, channel: 'Generated activity', instrument: base.destinationId, status: 'Recorded' },
  ];
  ...
  return {
    ...base,
    primaryObject: base.destinationId,
    secondaryObject: base.bankCode,
    objectLabels: ['Destination ID', 'Bank Code'],
    objectSummary: [
      { value: `Destination ID · ${base.destinationId}`, detail: base.verificationStatus },
      { value: `Bank Code · ${base.bankCode}`, detail: base.accountType },
