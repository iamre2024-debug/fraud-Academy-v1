...
import { assertNoHiddenFindingLeak } from './hiddenFindingGuard.js';

export const CASE_MIGRATION_VERSION = CASE_DOMAIN_VERSION;
export const CASE_MIGRATION_IDS = Object.freeze({
  HIDDEN_FINDING_FREEZE: 'hidden-finding-freeze-v1',
  PAYROLL_WORKFLOW_REASSIGNMENT: 'payroll-workflow-reassignment-v1',
  HISTORICAL_PAYROLL_DESTINATIONS: 'historical-payroll-destinations-v1',
  KYB_TO_BUSINESS_360: 'kyb-to-business-360-v1',
});
...
const payrollCredentialOrSessionCompromisePattern = /\b(?:credential(?:s)? (?:compromised|stolen|reset|changed)|compromised credential(?:s)?|unauthorized (?:login|session|password reset|mfa change)|session (?:compromise|hijack|takeover)|account takeover)\b/i;
...
function explicitPayrollCompromiseEvents(record = {}) {
  const eventCollections = [
    record.events,
    record.timelineEvents,
    record.loginHistory,
    record.sessionHistory,
    record.toolResults?.loginHistory,
    record.toolResults?.sessionHistory,
    record.toolResults?.systemAccess,
  ];
  return eventCollections
    .flatMap((records) => (Array.isArray(records) ? records : []))
    .filter((event) => payrollCredentialOrSessionCompromisePattern.test([
      event?.eventType,
      event?.label,
      event?.title,
      event?.detail,
...
function payrollCreditWorkflowReassignment(record = {}) {
  if (
    record.productType !== PRODUCT_TYPES.PAYROLL_PRODUCT
    || ![WORKFLOW_TYPES.CREDIT_APPLICATION_REVIEW, WORKFLOW_TYPES.CREDIT_RISK_REVIEW]
      .includes(record.workflowType)
  ) return null;
  const compromiseEvents = explicitPayrollCompromiseEvents(record);
  return {
    migrationId: CASE_MIGRATION_IDS.PAYROLL_WORKFLOW_REASSIGNMENT,
    fromWorkflowType: record.workflowType,
    toWorkflowType: compromiseEvents.length
      ? WORKFLOW_TYPES.PAYROLL_ACCOUNT_TAKEOVER
      : WORKFLOW_TYPES.PAYROLL_CHANGE_ALERT,
    reason: compromiseEvents.length
      ? 'Explicit credential or session-compromise event recorded.'
      : 'Payroll credit workflow retired; deterministic default applied.',
    supportingRecordIds: compromiseEvents.map((event) => cleanText(event?.id)).filter(Boolean),
  };
}
...
export function classifyLegacyCase(record = {}) {
  const payrollReassignment = payrollCreditWorkflowReassignment(record);
  if (payrollReassignment) {
    return {
      customerType: CUSTOMER_TYPES.BUSINESS,
      productType: PRODUCT_TYPES.PAYROLL_PRODUCT,
      workflowType: payrollReassignment.toWorkflowType,
    };
  }
...
function domainForLegacyFindingDerivation(record = {}) {
  if (
    record.productType === PRODUCT_TYPES.PAYROLL_PRODUCT
    && [WORKFLOW_TYPES.CREDIT_APPLICATION_REVIEW, WORKFLOW_TYPES.CREDIT_RISK_REVIEW]
      .includes(record.workflowType)
    ...
    return {
      customerType: record.customerType ?? CUSTOMER_TYPES.BUSINESS,
      productType: PRODUCT_TYPES.PAYROLL_PRODUCT,
      workflowType: record.workflowType,
    };
  }
  return classifyLegacyCase(record);
}
...
function legacyCaseMetadata(record = {}, workflowReassignment = null) {
  const priorReassignments = Array.isArray(record.legacyMetadata?.workflowReassignments)
    ? record.legacyMetadata.workflowReassignments
    : [];
  const workflowReassignments = workflowReassignment
    ? [
        ...priorReassignments.filter((item) => (
          item?.migrationId !== workflowReassignment.migrationId
          || item?.fromWorkflowType !== workflowReassignment.fromWorkflowType
        )),
        workflowReassignment,
      ]
    : priorReassignments;
  return {
    ...
    ...(workflowReassignments.length ? { workflowReassignments } : {}),
  };
}
...
function destinationAppliesToPayDate(destination = {}, payDate) {
  const introduced = destination.firstSeen
    ?? destination.effectiveDate
    ?? destination.introducedDate
    ?? destination.destinationIntroducedDate;
  if (!introduced) {
    return !(
      destination.copiedFromEmployeeProfile === true
      || destination.historicalDestinationOverwritten === true
      || /employee[- ]current|current employee profile/i.test(cleanText(destination.source))
    );
  }
  const introducedAt = migrationDate(introduced);
  const payDateAt = migrationDate(payDate);
  return introducedAt === null || payDateAt === null || introducedAt <= payDateAt;
}

function historicalPaymentPlan(employeeProfile = {}, payDate) {
  return [...(employeeProfile.paymentHistory ?? [])]
    .filter((plan) => {
      const effectiveAt = migrationDate(plan?.effectiveDate);
      const payDateAt = migrationDate(payDate);
      return effectiveAt !== null && payDateAt !== null && effectiveAt <= payDateAt;
    })
    .sort((left, right) => migrationDate(left.effectiveDate) - migrationDate(right.effectiveDate))
    .at(-1) ?? null;
}

function unavailableDestination(paystub = {}, payDate) {
  return {
    id: `${paystub.id ?? 'PAYSTUB'}-DESTINATION-UNAVAILABLE`,
    method: 'Historical destination unavailable',
    bankCode: 'Unavailable',
    destinationId: 'Unavailable',
    amount: Number(paystub.summary?.netPay ?? paystub.netPay ?? 0),
    status: paystub.settlementStatus ?? 'Historical record gap',
    settlementDate: paystub.settlementDate ?? payDate,
    paymentRecordId: null,
    firstSeen: payDate,
    destinationUnavailable: true,
    unavailableReason: 'The event-specific historical destination could not be recovered. No replacement value was fabricated.',
  };
}

function destinationsFromHistoricalPlan(plan, paystub, payDate) {
  if (!plan) return [unavailableDestination(paystub, payDate)];
  if (plan.method === 'Paper check') {
    return [{
      id: `${paystub.id ?? 'PAYSTUB'}-CHECK`,
      method: 'Paper check',
      ...
      settlementDate: paystub.settlementDate ?? payDate,
      paymentRecordId: plan.paymentRecordId ?? null,
      checkNumber: plan.checkNumber ?? 'Unavailable',
      firstSeen: plan.effectiveDate,
    }];
  }
  const destinations = Array.isArray(plan.destinations) ? plan.destinations : [];
  if (!destinations.length) return [unavailableDestination(paystub, payDate)];
  const netPay = Number(paystub.summary?.netPay ?? paystub.netPay ?? 0);
  let allocated = 0;
  return destinations.map((destination, index) => {
    const amount = index === destinations.length - 1
      ? Math.round((netPay - allocated) * 100) / 100
      : Math.round(Number(destination.amount ?? (netPay * Number(destination.percentage ?? 0))) * 100) / 100;
    allocated = Math.round((allocated + amount) * 100) / 100;
    return {
      id: destination.id ?? `${paystub.id ?? 'PAYSTUB'}-PMT-${index + 1}`,
      method: plan.method ?? (destinations.length > 1 ? 'Split direct deposit' : 'Direct deposit'),
...
function migratePaystubDestinations(paystub, payDate, employeeProfile) {
  if (!paystub || typeof paystub !== 'object') return paystub;
  const destinations = Array.isArray(paystub.paymentDestinations)
    ? paystub.paymentDestinations
    : [];
  if (
    destinations.length
    && destinations.every((destination) => (
      destination.destinationUnavailable === true
      || destinationAppliesToPayDate(destination, payDate)
    ))
  ) return paystub;
  const paymentPlan = historicalPaymentPlan(employeeProfile, payDate);
  return {
    ...paystub,
    paymentDestinations: destinationsFromHistoricalPlan(paymentPlan, paystub, payDate),
    destinationMigration: {
      migrationId: CASE_MIGRATION_IDS.HISTORICAL_PAYROLL_DESTINATIONS,
      resolvedFrom: paymentPlan ? 'event-specific employee payment history' : 'destinationUnavailable',
    },
  };
}

function migratePayrollRuns(runs = [], employeeProfilesById) {
  if (!Array.isArray(runs)) return runs;
  return runs.map((run) => ({
    ...run,
    employees: Array.isArray(run.employees)
      ? run.employees.map((employee) => {
          const employeeId = employee.employeeId ?? employee.id ?? employee.paystub?.employee?.employeeId;
          const paystub = migratePaystubDestinations(
            employee.paystub,
            run.payDate ?? employee.paystub?.payDate,
            employeeProfilesById.get(employeeId) ?? employee,
          );
          return paystub === employee.paystub ? employee : { ...employee, paystub };
        })
      : run.employees,
  }));
}

export function migrateHistoricalPayrollDestinations(record = {}) {
  if (!record || typeof record !== 'object' || Array.isArray(record)) return record;
  const employeeProfiles = [
    ...(Array.isArray(record.employeeProfiles) ? record.employeeProfiles : []),
    ...(Array.isArray(record.toolResults?.employeeProfile) ? record.toolResults.employeeProfile : []),
  ];
  const employeesById = new Map(employeeProfiles
    .filter((employee) => employee?.id || employee?.employeeId)
    .map((employee) => [employee.id ?? employee.employeeId, employee]));
  const next = { ...record };
  if (Array.isArray(record.payrollRuns)) {
    next.payrollRuns = migratePayrollRuns(record.payrollRuns, employeesById);
  }
  if (record.toolResults && typeof record.toolResults === 'object') {
    next.toolResults = { ...record.toolResults };
    if (Array.isArray(record.toolResults.payrollRuns)) {
      next.toolResults.payrollRuns = migratePayrollRuns(record.toolResults.payrollRuns, employeesById);
    }
    if (Array.isArray(record.toolResults.payrollHistory)) {
      next.toolResults.payrollHistory = migratePayrollRuns(record.toolResults.payrollHistory, employeesById);
    }
  }
...
export function migrateGeneratedCase(record) {
  ...
  if (
    ...
    const migratedHistory = migrateHistoricalPayrollDestinations(next);
    return JSON.stringify(migratedHistory) === JSON.stringify(record) ? record : migratedHistory;
  }
  const workflowReassignment = payrollCreditWorkflowReassignment(record);
  ...
  const next = {
    ...
    scoringRules: record.scoringRules
      ...
      : record.scoringRules,
    legacyMetadata: legacyCaseMetadata(record, workflowReassignment),
  };
  ...
  const compactNext = compactSerializableValue(migrateHistoricalPayrollDestinations(next));
...
export function publicGeneratedCaseRecord(record) {
  ...
  const publicRecord = compactSerializableValue({
    ...persisted,
    ...(legacyPresentation
      ? publicPresentationFields(persisted)
      : publicEvidenceFields(persisted)),
  });
  return assertNoHiddenFindingLeak(publicRecord, 'public generated case');
}
...
export function migrateLegacyCaseTruth(record = {}) {
  ...
  const domain = domainForLegacyFindingDerivation(record);
  ...
  return compactSerializableValue({
    ...
    operationalDecision,
    finalFinding,
    hiddenFinding: finalFinding,
    findingDerivationMigrationId: CASE_MIGRATION_IDS.HIDDEN_FINDING_FREEZE,
    findingBasis,
...
export function normalizeQuickPadByCase(saved = {}) {
  ...
  return Object.fromEntries(Object.entries(saved).map(([caseId, quickPad]) => {
    ...
    const normalizedItems = Array.isArray(quickPad.items)
      ? quickPad.items.map((item) => {
          if (!item || typeof item !== 'object' || Array.isArray(item)) return item;
          const sourceTool = normalizeToolName(item.sourceTool ?? item.source);
          return {
            ...item,
            ...(item.sourceTool !== undefined ? { sourceTool } : {}),
            ...(item.source !== undefined ? { source: sourceTool } : {}),
            id: sourceTool
              ? `${sourceTool}:${item.label ?? ''}:${item.value ?? ''}`
              : item.id,
          };
        })
      : quickPad.items;
    const dedupedItems = Array.isArray(normalizedItems)
      ? [...new Map(normalizedItems.map((item, index) => [
          item && typeof item === 'object'
            ? item.id ?? `${item.sourceTool ?? item.source ?? ''}:${item.label ?? ''}:${item.value ?? ''}`
...
export function migratePersistenceResources(rawByKey = {}, generatedCases = []) {
  const storedSchemaVersion = Number(rawByKey[storageKeys.schemaVersion]?.global?.version) || 0;
  if (storedSchemaVersion >= CASE_MIGRATION_VERSION) {
    return {
      rawByKey,
      generatedCases,
    };
  ...
  nextRaw[storageKeys.schemaVersion] = {
    global: {
      version: CASE_MIGRATION_VERSION,
      migrations: { ...CASE_MIGRATION_IDS },
    },
  };
...
export function migrateCloudSnapshotCaseData(snapshot = {}) {
  const storedSchemaVersion = Number(
    snapshot.resources?.[storageKeys.schemaVersion]?.entries?.global?.value?.version,
  ) || 0;
  if (storedSchemaVersion >= CASE_MIGRATION_VERSION) return snapshot;
  ...
  const existingSchemaEntry = resources[storageKeys.schemaVersion]?.entries?.global;
  resources[storageKeys.schemaVersion] = {
    mode: 'value',
    entries: {
      ...(resources[storageKeys.schemaVersion]?.entries ?? {}),
      global: {
        ...existingSchemaEntry,
        value: {
          version: CASE_MIGRATION_VERSION,
        ...
        version: existingSchemaEntry?.version ?? { at: 1, deviceId: 'case-migration' },
        deleted: false,
      },
    },
  };
