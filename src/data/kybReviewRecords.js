...
function ownerProfile(owner, index, profile, checkedDate) {
  const seed = stableNumber(`${owner.id}-${owner.name}`);
  const nameParts = String(owner.name).trim().split(/\s+/);
  const firstName = nameParts[0] || 'Training';
  const lastName = nameParts.at(-1) || 'Owner';
  const isControllingParty = profile.ownership.controllingParty.identityReference === owner.identityReference;
  return {
    ...owner,
    legalName: owner.legalName ?? owner.name,
    dateOfBirth: owner.dateOfBirth ?? `${['Jan', 'Mar', 'May', 'Jul', 'Sep'][seed % 5]} ${2 + (seed % 25)}, ${1972 + (seed % 24)}`,
    trainingId: owner.trainingId ?? `TRN-OWN-${String(seed).padStart(6, '0').slice(-6)}`,
    title: owner.title ?? owner.role,
    controlStatus: owner.controlStatus ?? (isControllingParty ? 'Controlling party on file' : 'Beneficial owner; not the recorded control person'),
    guarantorStatus: owner.guarantorStatus ?? 'No personal guarantor role recorded',
    phone: owner.phone ?? `(555) 02${String(seed % 100).padStart(2, '0')}-${String(seed).padStart(4, '0').slice(-4)}`,
    email: owner.email ?? `${firstName}.${lastName}.${String(seed).slice(-3)}@owner.training.example`.toLowerCase(),
    currentResidentialAddress: owner.currentResidentialAddress
      ?? `${300 + (seed % 7000)} ${['Juniper', 'Willow', 'Cedar', 'Lakeview'][seed % 4]} Residence Lane, Dallas, TX 75201`,
    previousResidentialAddress: owner.previousResidentialAddress
      ?? `${100 + ((seed + index + 17) % 7000)} ${['Archive', 'Crescent', 'Parkview', 'Harbor'][(seed + 1) % 4]} Training Road, Arlington, TX 76010`,
    verificationDetails: owner.verificationDetails ?? `${owner.verificationStatus}; owner identity, role, and address records compared.`,
    personalAccounts: owner.personalAccounts ?? [{
      id: `OWN-ACCT-${String(seed).padStart(6, '0').slice(-6)}`,
      product: seed % 2 ? 'Personal checking' : 'Personal credit card',
      status: 'Open · good standing',
    }],
    trustedDevices: owner.trustedDevices ?? [{
      id: `DEV-OWN-${String(seed).padStart(6, '0').slice(-6)}`,
      device: seed % 2 ? 'Trusted mobile phone' : 'Trusted personal computer',
      status: 'Trusted on personal profile',
      lastSeen: checkedDate,
    }],
    contactHistory: owner.contactHistory ?? [{
      id: `CONTACT-OWN-${String(seed).padStart(6, '0').slice(-6)}`,
      date: checkedDate,
      channel: 'Secure message',
      note: 'Personal contact information reviewed; no business profile field was changed.',
    }],
  };
}
...
function recordsFor(profile) {
  ...
  const ownership = [
    ...profile.ownership.beneficialOwners.map((owner) => sourceRecord({
      ...
      fields: [
        ...
        ['Verification details', owner.verificationDetails],
        ['Personal accounts', owner.personalAccounts.map((account) => `${account.id} · ${account.product} · ${account.status}`).join(' · ')],
        ['Trusted devices', owner.trustedDevices.map((device) => `${device.id} · ${device.device} · ${device.status}`).join(' · ')],
        ['Contact history', owner.contactHistory.map((contact) => `${contact.id} · ${contact.date} · ${contact.channel} · ${contact.note}`).join(' · ')],
        ['First recorded', owner.firstRecorded],
      ],
      relatedRecords: [owner.trainingId, ...owner.personalAccounts.map((account) => account.id), ...owner.trustedDevices.map((device) => device.id), profile.registrationId],
    })),
  ...
  const relationship = [sourceRecord({
    id: `${profile.businessId}-RELATIONSHIP`,
    title: 'Business relationship summary',
    category: 'Institution relationship',
    value: profile.relationship.clientDuration,
    observed: profile.relationship.relationshipStartDate,
    detail: 'Reusable relationship, servicing, access, credit, and payroll facts retained independently of any current claim.',
    fields: [
      ['Client duration', profile.relationship.clientDuration],
      ['Credit and loan details', profile.relationship.creditAndLoanDetails.join(' · ')],
    ...
    relatedRecords: profile.relationship.accounts.map((account) => account.id),
  }), ...profile.relationship.accounts.map((account) => sourceRecord({
    id: account.id,
...
function makeProfile(input) {
  ...
  const baseProfile = {
    ...
    relationship: {
      ...input.relationship,
      clientDuration: input.relationship.clientDuration ?? businessAge(input.relationship.relationshipStartDate, checkedDate),
      creditAndLoanDetails: input.relationship.creditAndLoanDetails
        ?? input.relationship.accounts
          .filter((account) => /credit|loan|line/i.test(account.product))
          .map((account) => `${account.product} · ${account.status} · balance ${account.balance} · limit ${account.limit}`),
      profileChanges: input.relationship.profileChanges ?? ['No reusable business-profile change supplied for this training record'],
      accessSecurity: input.relationship.accessSecurity ?? ['Authorized users and trusted devices are maintained in the relationship record'],
      contactNotes: input.relationship.contactNotes ?? ['Annual relationship review recorded by business servicing'],
      payrollRelationshipSummary: input.relationship.payrollRelationshipSummary
        ?? (input.relationship.accounts.some((account) => /payroll/i.test(account.product))
          ? 'Active payroll relationship; open Payroll History for dated run and paystub records'
          : 'No payroll product recorded for this business profile'),
    },
  };
  const profile = {
    ...baseProfile,
    ownership: {
      ...
      beneficialOwners: baseProfile.ownership.beneficialOwners.map((owner, index) => (
        ownerProfile(owner, index, baseProfile, checkedDate)
      )),
    },
  };
...
// Compatibility adapter for the reusable Business 360 dossier. Standalone
// KYB navigation is retired, but saved business-source records still use this
// neutral shape while they are migrated into Business 360.
