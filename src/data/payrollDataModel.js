...
function paymentStatusForRun(runStatus, configuredStatus) {
  if (/pending|failed|returned|reversed/i.test(String(runStatus ?? ''))) return runStatus;
  return configuredStatus;
}

function allocateDestinations(plan, netPay, paystubId, settlementDate, runStatus) {
  if (!plan || plan.method === 'Paper check') {
    return [{
      ...
      amount: cents(netPay),
      status: paymentStatusForRun(runStatus, plan?.status ?? 'Issued'),
      settlementDate,
  ...
  return configured.map((destination, index) => {
    ...
    return {
      ...
      amount,
      status: paymentStatusForRun(runStatus, destination.status ?? plan.status ?? 'Settled'),
      settlementDate,
...
function employeePaystub({ company, employee, run, ytdState, runIndex }) {
  ...
  const paymentDestinations = allocateDestinations(paymentPlan, netPay, paystubId, run.settlementDate, run.status);
  ...
  return {
    ...
    reimbursements,
    adjustments,
    paymentMethod: paymentDestinations.length > 1
      ? 'Split direct deposit'
      : paymentDestinations[0]?.method ?? 'Not recorded',
    settlementStatus: run.status ?? 'Completed',
    paymentDestinations,
    summary: {
      grossPay,
      employeeTaxes,
  ...
  const payrollRuns = sortedRuns.map((run, runIndex) => {
    ...
    return {
      ...
      fundingStatus: run.fundingStatus ?? (
        /pending/i.test(run.status ?? '')
          ? 'Pending funding'
          : /failed/i.test(run.status ?? '')
            ? 'Failed funding'
            : /returned/i.test(run.status ?? '')
              ? 'Returned funding'
              : /reversed/i.test(run.status ?? '')
                ? 'Reversed funding'
                : 'Funded'
      ),
      companyFunding: {
