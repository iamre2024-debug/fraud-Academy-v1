...
function makeBusinessRecords({ id, claimType, scenario, person, employer, business, parties = [], reportedDate, issueStartDate, amount, recordCount, creditProfile, applicationVerification = [] }) {
  ...
  if (tools.has('KYB Review')) {
    const verificationParties = parties.filter((party) => /submitter|owner|control person|guarantor|administrator/i.test(party.role));
    result.businessIntel = [
      ...
      ...verificationParties.map((party, partyIndex) => ({
        id: `${id}-BIN-${partyIndex + 2}`,
        type: party.role,
        value: party.name,
        observed: reportedDate,
        context: `${party.name} is separately identified as ${party.role.toLowerCase()} and requires role-appropriate verification and link analysis.`,
      })),
      { id: `${id}-BIN-${verificationParties.length + 2}`, type: 'Operating and revenue context', value: creditProfile?.statedAnnualIncome ?? money(Math.max(amount * 18, 85000)), observed: reportedDate, context: `Stated annual revenue is compared with ${scenario.amount} in current exposure.` },
      { id: `${id}-BIN-${verificationParties.length + 3}`, type: 'Case activity', value: scenario.transactionInfo, observed: issueStartDate, context: `${scenario.subtype} activity was reported through ${scenario.channel} on ${reportedDate}.` },
...
export function buildGeneratedToolResults({ id, index, person, city, employer, business, phone, email, address, parties = [], claimType, scenario, documents, recordCount, trainingId, reportedDate, issueStartDate, difficulty }) {
  ...
  const result = {
    ...
    paymentVerification,
    applicationVerification,
    ...makeBusinessRecords({ id, claimType, scenario, person, employer, business, parties, reportedDate, issueStartDate, amount, recordCount, creditProfile, applicationVerification }),
    ...evidence,
