...
export const businessIntelSearchModes = {
  businessId: {
    ...
    expected(profile = {}) {
      return profile.registrationFileNumber;
    },
...
export function matchesBusinessIntelSearch(dossier = {}, criteria = null) {
  ...
  const expectedSecondary = mode.normalize(mode.expected(profile));
  ...
  return Boolean(
    ...
    && expectedSecondary === submittedSecondary
  );
}
...
export function prefillBusinessIntelSearch(dossier = {}, routedQuery = '') {
  ...
  const match = Object.entries(businessIntelSearchModes).find(([, mode]) => {
    const expected = mode.normalize(mode.expected(profile));
    return expected && expected === mode.normalize(query);
  });
