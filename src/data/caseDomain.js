...
export function filterToolsForCaseDomain(toolNames = [], domain = {}) {
  ...
  const isPayrollWorkflow = domain.productType === PRODUCT_TYPES.PAYROLL_PRODUCT
    || payrollWorkflows.has(domain.workflowType);

  return normalizedTools.filter((toolName) => {
    if (isPersonal && toolName === 'Business 360' && !hasLinkedBusiness) return false;
    if (isPersonal && payrollOnlyTools.has(toolName) && !isPayrollReview) return false;
