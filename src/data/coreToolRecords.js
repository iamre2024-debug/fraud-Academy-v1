...
export function buildCoreToolRecords(tool, activeCase, fallbackData = { rows: [] }) {
  ...
  const financial = financialRecordsByCase[activeCase.id];
  const business = businessRecordsByCase[activeCase.id];
  const evidence = evidenceRecordsByCase[activeCase.id];

  if (tool === 'Payment Verification') {
    if (!financial) return fallbackData;
    return {
  ...
  if (tool === 'Business Intelligence') {
    if (!business) return fallbackData;
    return {
  ...
  if (tool === 'Evidence Center') {
    if (!evidence) return fallbackData;
    return {
  ...
  if (tool === 'Link Analysis') {
    if (!financial && !business && !evidence) return fallbackData;
    return {
  ...
  if (tool === 'Timeline') {
    if (!financial && !evidence) return fallbackData;
    return {
