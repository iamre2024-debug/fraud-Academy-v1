...
export function migrateReviewPackages(saved = {}) {
  return Object.fromEntries(Object.entries(saved).map(([caseId, packages]) => [
    ...
    (packages ?? []).map((reviewPackage) => ({
      ...
      requiredTools: canonicalToolNames(reviewPackage.requiredTools ?? []),
...
export default function useVisualWorkspaceCaseState(activeCase) {
  ...
  const [completedByCase, setCompletedByCase] = useState(() => migrateCompletedTools(normalizeCompletedToolsByCase(readStorage(storageKeys.completed, {}))));
  ...
  const [quickPadByCase, setQuickPadByCase] = useState(() => migrateQuickPads(normalizeQuickPadByCase(readStorage(storageKeys.quickPad, {}))));
  ...
  useEffect(() => {
    const hydrateFromRecovery = () => {
      ...
      setCompletedByCase(migrateCompletedTools(normalizeCompletedToolsByCase(readStorage(storageKeys.completed, {}))));
      ...
      setQuickPadByCase(migrateQuickPads(normalizeQuickPadByCase(readStorage(storageKeys.quickPad, {}))));
