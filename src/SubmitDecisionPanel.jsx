...
export default function SubmitDecisionPanel({
  ...
  const selectedOperational = decisionDraft.operationalDecision || decisionDraft.choice || '';
  const selectedFinding = decisionDraft.finalFinding || '';
  const hasSelections = Boolean(selectedOperational && selectedFinding);
  const [editorOpen, setEditorOpen] = useState(!hasSelections);

  useEffect(() => {
    setEditorOpen(!hasSelections);
  }, [activeCase.id]);

  const evidenceCards = useMemo(
    () => tray.slice(0, 3).map(describePin),
    [tray],
  );
  const latestUsefulNote = useMemo(
    () => cleanNote(notes.find(isUsefulNote) ?? notes[0] ?? ''),
    [notes],
  );
    ...
    >
      ...
      <section className="decision-selected-card" aria-label="Selected decision">
        ...
        <div className="decision-selected-summary">
          ...
          <em>{decisionDraft.confidence || 'Medium'} confidence</em>
        </div>
        <dl>
          <div><dt>Amount / exposure</dt><dd>{activeCase.amountExposure ?? activeCase.amount ?? 'Not supplied'}</dd></div>
          <div><dt>Reviewed tools</dt><dd>{packageStatus.reviewedRequired}/{packageStatus.totalRequired}</dd></div>
        </dl>
      </section>
      ...
      <section className="decision-evidence-card" aria-label={`Pinned evidence (${tray.length})`}>
        <header>
          <ReviewGlyph type="pin" />
          <h3>Pinned Evidence ({tray.length})</h3>
        </header>
